...
export default function Home() {
  const [oem,setOem]=useState("8K0615301M"), [marketplace,setMarketplace]=useState("EBAY_US"), [condition,setCondition]=useState<"ANY"|"NEW"|"USED">("ANY");
  ...
  return <main>
    <header>
      <div><span className="eyebrow">AUTOMOTIVE OPERATIONS</span><h1>Part<span>Pulse</span></h1></div>
    ...
    {result&&<section className="results"><div className="resultTitle"><div><span className="eyebrow">VERIFIED MARKET SNAPSHOT</span><h3>{result.oem}</h3></div><span>{result.marketplace.replace("EBAY_","")} · {result.conditionFilter === "ANY" ? "ALL CONDITIONS" : result.conditionFilter} · {new Date(result.searchedAt).toLocaleString()}</span></div>
      ...
      <div className="tableWrap"><table><thead><tr><th>LISTING</th><th>SELLER</th><th>CONDITION</th><th>ITEM</th><th>SHIPPING</th><th>LANDED PRICE</th></tr></thead><tbody>{result.listings.map(item=><tr key={item.id}><td><a href={item.url} target="_blank">{item.title}</a><span className="listingId">eBay ID: {ebayListingId(item.id)}</span></td><td>{item.seller}</td><td><span className="pill">{item.condition}</span></td><td>{money(item.price,item.currency)}</td><td>{money(item.shipping,item.currency)}</td><td><b>{money(item.landedPrice,item.currency)}</b></td></tr>)}</tbody></table></div></>:<div className="empty">No exact verified competitor matches found.</div>}
    </section>}
